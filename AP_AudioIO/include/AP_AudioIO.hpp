...
private:
    ...
    bool readHeaderData(char buffer[], int32_t start_point);
    ...
    /**
     * True if action successful, False in case it is not. 
    */
    bool removeUnusedChuncks(std::ifstream& _file, char (& _buffer)[44], 
        int32_t& current_pos, int32_t& start_point);
};
